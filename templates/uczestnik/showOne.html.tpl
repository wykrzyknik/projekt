...
{block name=title}Szczegółowe informacje o uczestnik{/block}
...
<div class="row">
  <div class="col-sm-6 col-md-4 col-sm-offset-3 col-md-offset-4">
    <div class="thumbnail">
      <div class="caption">
        <h4><span style="color:red">Id</span> {$data['id']} </br> <span style="color:red">Nazwisko</span>  {$data['Nazwisko']} </br> <span style="color:red">Imie</span> {$data['Imie']} </br> <span style="color:red">Data_ur</span>  {$data['Data_ur']} </br> <span style="color:red">PESEL</span> {$data['PESEL']} </br> <span style="color:red">Kod_pocztowy</span> {$data['Kod_pocztowy']} </br> <span style="color:red">Miasto</span> {$data['Miasto']} </br> <span style="color:red">Ulica</span> {$data['Ulica']} </br> <span style="color:red">Numerlokalu</span> {$data['Numerlokalu']} </br> <span style="color:red">Telefon</span> {$data['Telefon']} </br> <span style="color:red">Email</span> {$data['Email']} </br></h3>
        <p class="text-right">
          <a href="{$protocol}{$smarty.server.HTTP_HOST}{$subdir}uczestnik/usun/{$data['id']}" type="button" class="btn btn-danger btn-sm delete-button"
